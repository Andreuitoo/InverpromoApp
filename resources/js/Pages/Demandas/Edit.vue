  ...
  <div>
    ...
    <div class="flex justify-start mb-8 max-w-3xl">
      <h1 class="text-3xl font-bold">
        <Link class="text-green-700 hover:text-green-500" href="/demandas">Demandas</Link>
        <span class="text-green-700 font-medium">/</span>
        {{ form.ref }}
      </h1>
    </div>
    <trashed-message v-if="demanda.deleted_at" class="mb-6" @restore="restore"> Esta demanda ha sido eliminada. </trashed-message>
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        <div class="flex flex-wrap -mb-8 -mr-6 p-8">
          <text-input v-model="form.ref" :error="form.errors.ref" class="pb-8 pr-6 w-full lg:w-1/2" placeholder="DXXXX" label="Refrencia" />
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button v-if="!demanda.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Eliminar demanda</button>
          <loading-button :loading="form.processing" class="btn-indigo ml-auto" type="submit">Actualizar demanda</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      form: this.$inertia.form({
        _method: 'put',
        ref: this.demanda.ref,
        nombre: this.demanda.nombre,
        telefono: this.demanda.telefono,
        telefono_2: this.demanda.telefono_2,
        zona: this.demanda.zona,
        zona_2: this.demanda.zona_2,
        num_hab: this.demanda.num_hab,
        num_hab_2: this.demanda.num_hab_2,
        precio: this.demanda.precio,
        precio_2: this.demanda.precio_2,
        fecha: this.demanda.fecha,
  ...
  methods: {
    update() {
      this.form.post(`/demandas/${this.demanda.id}`)
    },
    destroy() {
      if (confirm('Estas seguro que quieres borrar esta demanda?')) {
        this.$inertia.delete(`/demandas/${this.demanda.id}`)
      }
    },
    restore() {
      if (confirm('Estas seguro que quieres restaurar esta demanda?')) {
        this.$inertia.put(`/demandas/${this.demanda.id}/restore`)
      }
    },
  },
}
